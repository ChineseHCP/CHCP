...
%  now support .gz files. Modified by Hai Li, 2013.12.6



function [hdr, filetype, fileprefix, machine] = load_nii_hdr_gz(fileprefix)

%  Check file extension. If .gz, unpack it into temp folder
%  added by Hai Li, 2013.12.6
   ...
   if length(fileprefix) > 2 & strcmp(fileprefix(end-2:end), '.gz')
	 ...
	 ~strcmp(fileprefix(end-6:end), '.nii.gz')
         ...
         error('Please check fileprefix.');
      end

      if str2num(version(1:3)) < 7.1 | ~usejava('jvm')
         error('Please use MATLAB 7.1 (with java) and above, or run gunzip outside MATLAB.');
      elseif strcmp(fileprefix(end-6:end), '.img.gz')
         fileprefix1 = fileprefix;
         fileprefix2 = fileprefix;
         fileprefix2(end-6:end) = '';
         fileprefix2 = [fileprefix2, '.hdr.gz'];

         tmpDir = tempname;
         mkdir(tmpDir);
         gzfileprefix = fileprefix;

         fileprefix1 = gunzip(fileprefix1, tmpDir);
         fileprefix2 = gunzip(fileprefix2, tmpDir);
         fileprefix = char(fileprefix1);	% convert from cell to string
      elseif strcmp(fileprefix(end-6:end), '.hdr.gz')
         fileprefix1 = fileprefix;
         fileprefix2 = fileprefix;
         fileprefix2(end-6:end) = '';
         fileprefix2 = [fileprefix2, '.img.gz'];
         ...
         fileprefix2 = gunzip(fileprefix2, tmpDir);
         fileprefix = char(fileprefix1);	% convert from cell to string
      elseif strcmp(fileprefix(end-6:end), '.nii.gz')
         tmpDir = tempname;
         mkdir(tmpDir);
         gzfileprefix = fileprefix;
         fileprefix = gunzip(fileprefix, tmpDir);
         fileprefix = char(fileprefix);	% convert from cell to string
      end
   end

   
   if ~exist('fileprefix','var'),
      error('Usage: [hdr, filetype, fileprefix, machine] = load_nii_hdr_gz(fileprefix)');
   end
